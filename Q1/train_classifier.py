"""
Task 1: Relation Extraction with Classification Head (Optimized for T4)
========================================================================
Key optimizations vs previous version:
- max_en_samples=15000 (was 60k) — focus on quality over quantity
- batch_size=32 with grad_accum=1 — maximize GPU throughput
- NO gradient checkpointing — faster forward/backward at cost of memory
- max_seq_len=150 — most NYT sentences fit
- Stratified sampling to ensure all labels represented
- 3 full epochs should complete in ~60-80 min on T4
"""
...
def load_label_maps(sft_dir):
    ...
    for f in os.listdir(sft_dir):
        if f.endswith("_map.json"):
            ...
            with open(os.path.join(sft_dir, f), "r", encoding="utf-8") as fh:
                m = json.load(fh)
            e2i[lang] = m
...
def build_labels(en_file, sft_dir):
    ...
    if os.path.isdir(sft_dir):
        for f in os.listdir(sft_dir):
            if f.endswith("_map.json"):
                with open(os.path.join(sft_dir, f), "r", encoding="utf-8") as fh:
                    labels.update(json.load(fh).keys())
    labels.discard("NA");
    labels.discard("")
    sl = ["NA"] + sorted(labels)
...
def load_samples(en_file, sft_dir, label2id, i2e, max_en):
    samples = []
    # English - stratified subsample
    en = []
    for e in load_jsonl(en_file):
        s = e.get("sentText", "")
        for rm in e.get("relationMentions", []):
    ...
    if len(en) > max_en:
        ...
        # Ensure minimum per label, then fill
        sub = []
        per = max(20, max_en // len(by_label))
        for lbl, items in by_label.items():
            random.shuffle(items)
            sub.extend(items[:per])
        rem = max_en - len(sub)
        if rem > 0:
            used = set(id(s) for s in sub)
            rest = [s for s in en if id(s) not in used]
            random.shuffle(rest)
            sub.extend(rest[:rem])
        en = sub[:max_en]
    samples.extend(en)
            ...
            if os.path.exists(fp):
                for e in load_jsonl(fp):
                    ...
                    for rm in e.get("relationMentions", []):
                        ...
                        indic.append(
                            {"sent": s, "em1": rm.get("em1Text", ""), "em2": rm.get("em2Text", ""), "label": el})
                break
    if indic and len(en) > 0:
        factor = min(20, max(1, len(en) // (5 * max(len(indic), 1))))
        indic = indic * factor
        print(f"  Indic oversampled {factor}x: {len(indic)}")
    samples.extend(indic)
...
class REDataset(Dataset):
    ...
    def __getitem__(self, i):
        s = self.samples[i]
        m = self.mark(s["sent"], s["em1"], s["em2"])
        enc = self.tok(m, max_length=self.maxlen, truncation=True, padding="max_length", return_tensors="pt")
        ids = enc["input_ids"].squeeze(0);
        mask = enc["attention_mask"].squeeze(0)
        e1id = self.tok.convert_tokens_to_ids("[E1S]")
        e2id = self.tok.convert_tokens_to_ids("[E2S]")
        e1p = (ids == e1id).nonzero(as_tuple=True)[0]
        e2p = (ids == e2id).nonzero(as_tuple=True)[0]
...
class REModel(nn.Module):
    def __init__(self, base, hs, nl, drop=0.1):
        super().__init__()
        self.base = base
        self.clf = nn.Sequential(nn.Linear(hs * 2, hs), nn.GELU(), nn.Dropout(drop), nn.Linear(hs, nl))
    ...
    def forward(self, ids, mask, e1, e2):
        ...
        p = torch.cat([h[b, e1], h[b, e2]], dim=-1)
        return self.clf(p)
...
def save_ckpt(model, peft, tok, l2i, i2l, e2i, cfg, odir, ep):
    ...
    for name, data in [("label2id.json", l2i), ("id2label.json", {str(k): v for k, v in i2l.items()}),
                       ("en_to_indic.json", e2i)]:
        with open(os.path.join(odir, name), "w", encoding="utf-8") as f: json.dump(data, f, ensure_ascii=False,
                                                                                   indent=2)
    with open(os.path.join(odir, "config.json"), "w") as f:
        json.dump({"model_name": cfg.model_name, "max_seq_len": cfg.max_seq_len,
                   "num_labels": len(l2i), "hidden_size": peft.config.hidden_size, "epoch": ep}, f, indent=2)
...
def train(cfg, odir, root):
    ...
    l2i, i2l = build_labels(en_file, sft_dir)
    nl = len(l2i)
    print(f"Labels: {nl}")
    ...
    samples = load_samples(en_file, sft_dir, l2i, i2e, cfg.max_en_samples)
    ...
    spe = len(dl)
    tot_steps = (spe // cfg.gradient_accumulation_steps) * cfg.num_epochs
    ...
    model = REModel(peft_model, peft_model.config.hidden_size, nl, cfg.lora_dropout).to(dev)
    model.clf.float()
    ...
    for ep in range(cfg.num_epochs):
        ...
        for bi, batch in enumerate(dl):
            ...
            e1 = batch["e1_pos"].to(dev, non_blocking=True);
            e2 = batch["e2_pos"].to(dev, non_blocking=True)
            lab = batch["label"].to(dev, non_blocking=True)
            if amp_on:
                with torch.amp.autocast('cuda', dtype=amp_dt):
                    logits = model(ids, mask, e1, e2).float()
                    loss = F.cross_entropy(logits, lab, weight=cw)
            else:
                logits = model(ids, mask, e1, e2).float()
                loss = F.cross_entropy(logits, lab, weight=cw)
            loss.backward()
            ...
            if (bi + 1) % 200 == 0:
                print(
                    f"  E{ep + 1}|B{bi + 1}/{spe}|L:{eloss / nb:.4f}|A:{cor / tot:.4f}|T:{(time.time() - t0) / 60:.1f}m")
        print(
            f"Epoch {ep + 1}|L:{eloss / nb:.4f}|A:{cor / tot:.4f}|T:{(time.time() - ep0) / 60:.1f}m|Tot:{(time.time() - t0) / 60:.1f}m")
        save_ckpt(model, peft_model, tok, l2i, i2l, e2i, cfg, odir, ep + 1)
